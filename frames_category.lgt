...
:- category(frames_category).
	...
	update_frame(OldFrames, Subject, [Pair|UpdatePairs], NewFrames) :-
		...
		::after_update(AccFrames0, Subject, Key, OldValue, NewValue, AccFrames),
	...
	delete_frame(OldFrames, Subject, NewFrames) :-
		...
		::after_delete_frame(AccFrames, Subject, NewFrames).
	...
	delete_frame(OldFrames, Subject, [Key-Value|DeletePairs], NewFrames) :-
		...
		::after_delete_slot(AccFrames0, Subject, Key, Value, AccFrames),
	...
	add_frame(OldFrames, Subject, Slots, NewFrames) :-
		...
		::after_add(AccFrames0, Subject, NewFrames).
	...
	add_frame_slots(OldFrames, Subject, [Key-NewValue|UpdatePairs], NewFrames) :-
		...
		::after_add(AccFrames0, Subject, Key, NewValue, AccFrames),
